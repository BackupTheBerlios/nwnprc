...
public final class Charconvert
{
    ...
    public static String convert(byte input[])
    {
        ...
        convert(input, 0, input.length, buffer, 0, input.length);
        return new String(buffer);
    }

    private static int convert(byte input[], int byteStart, int byteEnd, char output[], int charStart, int charEnd)
    {
        int max = byteEnd;
        ...
        if(max - byteStart > 10)
        {
            max -= 10;
            int byteOff;
            for(byteOff = byteStart; byteOff < max;)
            {
