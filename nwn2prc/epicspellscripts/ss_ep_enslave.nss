...
void main()
{
    ...
    if (GetCanCastSpell(OBJECT_SELF, SPELL_EPIC_ENSLAVE))
    {
        ...
        {
            //Make SR Check
            if (!PRCMyResistSpell(OBJECT_SELF, oTarget, GetTotalCastingLevel(OBJECT_SELF)+SPGetPenetr(OBJECT_SELF)) && !GetIsImmune(oTarget, IMMUNITY_TYPE_MIND_SPELLS) && !GetIsImmune(oTarget, IMMUNITY_TYPE_DOMINATE) && !GetIsPC(oTarget))
            {
                //Make a Will Save
