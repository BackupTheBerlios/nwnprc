...
namespace NWN.FileTypes
{
	...
	/// <summary>
	/// Class for all versions of an ErfKey
	/// </summary>
	public class ErfKey
	{
		/// <summary>
			...
			private struct ErfKeyVer10
		{
			...
			public ErfKeyVer10(string fileName)
			{
				unused = 0;
				resourceID = 0;
			...
			public static ErfKeyVer10[] Deserialize(Stream s, int count)
			{
				// Create an array of ErfKeys from the stream.
				ErfKeyVer10[] keys = new ErfKeyVer10[count];
				for (int i = 0; i < count; i++)
					keys[i] = new ErfKeyVer10(s);
				return keys;
			}
			...
			public static void Serlialize(Stream s, ErfKeyVer10[] keys)
			{
			...
			private struct ErfKeyVer11
		{
			...
			/// </summary>
			/// <param name="s">The stream</param>
			public ErfKeyVer11(Stream s)
			{
				// Let the raw serializer do the real work then just convert the
				// returned object to an ErfHeader.
				object o = RawSerializer.Deserialize(typeof(ErfKeyVer11), s);
				if (null == o) throw new NWNException("Invalid key in stream");
				this = (ErfKeyVer11) o;
			}
			...
			#region private fields/properties/methods
			[MarshalAs(UnmanagedType.ByValArray, SizeConst=32)] private byte[] resRef;
			private Int32 resourceID;
			private Int16 resType;
			private Int16 unused;
			#endregion
		...
		public string FileName 
		{
			...
			{
				if (version == 1.0)
					return keyVer10.FileName;
				else if (version == 1.1)
					return keyVer11.FileName;
				else
					throw new NWNException("Invalid version key");
			}
		}

		public void Serlialize(Stream s)
		{
			if (version == 1.0)
				keyVer10.Serialize(s);
			else if (version == 1.1)
				keyVer11.Serialize(s);
			else 
				throw new NWNException("Invalid version key");
		}
		...
		public int SizeOf()
		{
			return ErfKey.SizeOf(this.version);
		}
		...
		public static int SizeOf(double version)
		{
			if (version == 1.0)
				return Marshal.SizeOf(typeof(ErfKeyVer10));
			else if (version == 1.1)
				return Marshal.SizeOf(typeof(ErfKeyVer11));
			else
				throw new NWNException("Invalid version key");
		}
		#endregion
		...
		private ErfKeyVer10 keyVer10;
		private ErfKeyVer11 keyVer11;
		#endregion
...
NWNLogger.Log(0, "module.SaveAs updating object definition");
		...
		public static MemoryStream GetFile(string erf, string file)
		{
					   ...
					   new FileStream(erf, FileMode.Open, FileAccess.Read, FileShare.Read))
			{
				...
				ErfKey[] keys = ErfKey.Deserialize(reader, header.EntryCount, header.Version);
