...
namespace HakInstaller
{
	...
	public class HakInstaller
	{
			...
			string originalScript, StringCollection otherScripts)
		{
			...
			// SBH for NWN2
			// Use the PRC's compiler... its much better and can handle NWN2
			string compiler = Path.Combine(NWNInfo.ToolsPath, "nwnnsscomp.exe");
			b.Length = 0;
			// -c for compile, -I is the path to NWNSCRIPT.NSS 1 is the script to compile and 2 is the output directory for the ncs
			b.AppendFormat("-c -I \"{0}\" \"{1}\" \"{2}\"",NWNInfo.ToolsPath, fullSourceName, fullObjName);
