...
// Force move to exit WP and destroy self
void ForceExit(string sCreatureTag, string sWPTag, int bRun=FALSE)
{
	object oCreature = GetTarget(sCreatureTag);

	location lExitLoc = GetLocation(GetWaypointByTag(sWPTag));

	SetPlotFlag(oCreature,FALSE);
	AssignCommand(oCreature, SetIsDestroyable(TRUE));	// doing this in prep to destroy creature, so other params are irrelevant.
	
	AssignCommand(oCreature, ActionForceMoveToLocation(lExitLoc, bRun));
