...
//:://////////////////////////////////////////////
// ChazM 5/26/06 - added support for mundane and magic treasure 2da tables.
// ChazM 6/7/06 - added warlock and new prestige classes to DTSDetermineFeatToUse() (for special items)
// ChazM 7/7/06 - modified DTSCreateItemOnObject() - show error for bad rerefs
// ChazM 8/7/06 - DTSGenerateTreasureOnContainer() & DTSGenerateTreasureItems() now return number of treasures generated,
//					DTSDetermineFeatToUse() - removed superfluous return()
...
const string X2_DTS_2DA_MAGIC       =  "des_treas_magic";     	// 2da for magic items
const string X2_DTS_2DA_MUNDANE     =  "des_treas_mundane";       	// 2da for mundane items
...
int DTSGenerateTreasureOnContainer (object oContainer, object oOpener, int nClass, int nType = 5);
...
string DTSGet2DANameByType(int nType)
{
    ...
    else if (nType == X2_DTS_TYPE_GOLD)
    {
        sLookUp =  "X2_DTS_2DA_GOLD";
        sDefault = X2_DTS_2DA_GOLD;
    }
    else if (nType == X2_DTS_TYPE_MAGIC)
    {
        sLookUp =  "X2_DTS_2DA_MAGIC";
        sDefault = X2_DTS_2DA_MAGIC;
    }
    else if (nType == X2_DTS_TYPE_MUNDANE)
    {
        sLookUp =  "X2_DTS_2DA_MUNDANE";
        sDefault = X2_DTS_2DA_MUNDANE;
    }
...
object DTSCreateItemOnObject(string sItemTemplate, object oTarget)
{
    ...
    if (oItem == OBJECT_INVALID)
    {
       ...
       PrettyError(" +++2da ERROR +++ Invalid Entry:" + sItemTemplate);
    }
    return oItem;
}
...
int DTSDetermineFeatToUse(object oAdventurer, int bDontUseFeats = FALSE)
{
    ...
    switch (nHighest)
    {
		...
		case CLASS_TYPE_ELDRITCH_KNIGHT:
                    return FEAT_WEAPON_PROFICIENCY_WIZARD; // 51 -- class_wizard
                    break;
	...
	// ChazM 6/7/06 - 455 is prestige death attack 1 which maps to class_assassin; 
	// using 46 (weap prof simple) instead which maps to class_any 
	return 46; // class_any
}


// returns the number of "treasures" generated.
int DTSGenerateTreasureItems(object oContainer, object oOpener, int nClass, int nType)
{
   	string sItem;
	...
	int iNumGenerated = 0;	
   	...
   	if (nChance == 0)
        return iNumGenerated;     // no point in running further code
	...
	if ((nType & X2_DTS_TYPE_MUNDANE) && nChance > d100())
	{
		sItem = DTSGetRandomItemResRef(X2_DTS_TYPE_MUNDANE, nClass);
		DTSCreateItemOnObject(sItem, oContainer);
		iNumGenerated++;
	}
	if ((nType & X2_DTS_TYPE_MAGIC) && nChance > d100())
	{
		sItem = DTSGetRandomItemResRef(X2_DTS_TYPE_MAGIC, nClass);
		DTSCreateItemOnObject(sItem, oContainer);
		iNumGenerated++;
	}
...
int DTSGenerateTreasureOnContainer (object oContainer, object oOpener, int nClass, int nType = 5)
{
    ...
    {
        iNumGenerated += DTSGenerateTreasureItems(oContainer, oOpener,  nClass, nType);
    }
