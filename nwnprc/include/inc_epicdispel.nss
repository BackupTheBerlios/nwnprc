...
//:: This function is called from withing PRCSPApplyEffectToObject().  It's purpose is to
...
void spellsDispelMagicMod(object oTarget, int nCasterLevel, effect eVis, effect eImpac, int bAll = TRUE, int bBreachSpells = FALSE)
{
    ...
    DelayCommand(fDelay, SPApplyEffectToObject(DURATION_TYPE_INSTANT, eDispel, oTarget));
}
...
void spellsDispelAoEMod(object oTargetAoE, object oCaster, int nCasterLevel)
{
   ...
   if(d20(1) + nCasterLevel >= GetLocalInt(oTargetAoE, "X2_AoE_Caster_Level")+ModWeave)
   {
...
void DispelMagicBestMod(object oTarget, int nCasterLevel)
{
 ...
 //SendMessageToPC(GetFirstPC(), "Weave Caster:"+ IntToString(Weave));
  ...
  for(nCurrentEntry = 0; nCurrentEntry <= nLastEntry; nCurrentEntry++)
  {
    ...
    if(IsStillRealEffect(nEffectSpellID, oEffectCaster, oTarget))
    {
      ...
      int iDice = d20(1);
 //     SendMessageToPC(GetFirstPC(), "Dispell :"+ IntToString(iDice + nCasterLevel)+" vs DC "+IntToString(11 + nEffectCastLevel+ModWeave));
...
//  SendMessageToPC(GetFirstPC(), "Weave Caster:"+ IntToString(Weave));
  ...
  for(nIndex; nIndex < nLastEntry; nIndex++)
  {
    ...
    if(GetHasSpellEffect(nEffectSpellID, oTarget))
    {
      ...
      int iDice = d20(1);
//      SendMessageToPC(GetFirstPC(), "Dispell :"+ IntToString(iDice + nCasterLevel)+" vs DC "+IntToString(11 + nEffectCasterLevel+ModWeave));
