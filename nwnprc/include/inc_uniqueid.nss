...
string GetUniqueID()
{
    ...
    int i, max = array_get_size(oModule, PRC_UID_ARRAY);
    for(; i < max; i++)
        sReturn += IntToString(array_get_int(oModule, PRC_UID_ARRAY, i));

    // Increment the counters
    if((i = array_get_int(oModule, PRC_UID_ARRAY, max - 1)) < 0xFFFFFFFF)
        // We're below maximum integer size, just increment the stored value
       array_set_int(oModule, PRC_UID_ARRAY, max - 1, i + 1);
    else
        // We need to add a new integer to the set
        array_set_int(oModule, PRC_UID_ARRAY, max, 0);
