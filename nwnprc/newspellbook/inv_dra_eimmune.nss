...
void main()
{
    ...
    {
         RemoveSpellEffects(INVOKE_ENERGY_IMMUNITY_ACID, oCaster, oTarget);
    }
    if (GetHasSpellEffect(INVOKE_ENERGY_IMMUNITY_COLD,oTarget))
    {
         RemoveSpellEffects(INVOKE_ENERGY_IMMUNITY_COLD, oCaster, oTarget);
    }
    if (GetHasSpellEffect(INVOKE_ENERGY_IMMUNITY_ELEC,oTarget))
    {
         RemoveSpellEffects(INVOKE_ENERGY_IMMUNITY_ELEC, oCaster, oTarget);
    }
    if (GetHasSpellEffect(INVOKE_ENERGY_IMMUNITY_FIRE,oTarget))
    {
         RemoveSpellEffects(INVOKE_ENERGY_IMMUNITY_FIRE, oCaster, oTarget);
    }
    if (GetHasSpellEffect(INVOKE_ENERGY_IMMUNITY_SONIC,oTarget))
    {
         RemoveSpellEffects(INVOKE_ENERGY_IMMUNITY_SONIC, oCaster, oTarget);
    }
