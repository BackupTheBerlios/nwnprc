...
/*            // Keeps track of how many times we've been through here
        ...
        else if(nStage == STAGE_CONFIRMATION)//confirmation
        {
            if(nChoice == TRUE)
            {
                ...
                effect eDex  = EffectAbilityIncrease(GetLocalInt(oPC, "PRC_Power_AnimalAffinity_Stat"), 4);
                effect eLink = EffectLinkEffects(eDex, eDur);

                SPApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oPC, GetLocalFloat(oPC, "PRC_Power_AnimalAffinity_Duration"),
