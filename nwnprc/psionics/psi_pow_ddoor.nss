...
void main()
{
    ...
    if(!GetLocalInt(oCaster, "PRC_Spell_DimensionDoor_FirstStageDone"))
    {
        ...
        GetTeleportingObjects(oCaster, nManifesterLvl,
                              nSpellID == POWER_DIMENSIONDOOR_PARTY// ||
                              //nSpellID == SPELLID_TELEPORT_PARTY_DIRDIST
                              );

        if(nSpellID == POWER_DIMENSIONDOOR_SELFONLY ||
           nSpellID == POWER_DIMENSIONDOOR_PARTY
           )
        {
        ...
        {
            SpawnListener("psi_pow_ddoor_b", GetLocation(oCaster), "**", oCaster, 10.0f);
        ...
        SetLocalInt(oCaster, "PRC_Spell_DimensionDoor_CasterLvl", nManifesterLvl);
    }
    if(GetLocalInt(oCaster, "PRC_Spell_DimensionDoor_FirstStageDone"))
    {
