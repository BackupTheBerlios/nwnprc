...
// End of Spell Cast Hook
    ...
    object oTarget;
    int nLevel=GetLevelByClass(CLASS_TYPE_SHADOWLORD,OBJECT_SELF);
    ...
    while (GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
           ...
           if (!PRCDoResistSpell(OBJECT_SELF, oTarget,nLevel+SPGetPenetr(), fDelay))
           {
               int nDC = PRCGetSaveDC(oTarget,OBJECT_SELF);
                ...
                if (!PRCMySavingThrow(SAVING_THROW_WILL, oTarget, nDC, SAVING_THROW_TYPE_MIND_SPELLS, OBJECT_SELF, fDelay))
                {
                   ...
                   DelayCommand(fDelay, SPApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, RoundsToSeconds(nDuration),TRUE,-1,nLevel));
