...
// Converted to control Dragonfire Inspiration damage Jan 10, 2007 by Fox
...
DoDebug("x2_s3_flamingd: caster level = "+IntToString(PRCGetCasterLevel(oSpellOrigin)));
	...
	{
		int nDamageType = GetLocalInt(oWeapon, "Insp_Dam_Type");
		...
		// Get Caster Level
		int nAmount = GetLocalInt(oWeapon, "Insp_Dam_Dice");
		// Assume minimum caster level if variable is not found

		if (DEBUG) DoDebug("x2_s3_flamingd: on hit cast with weapon "+GetName(oWeapon)+", dice = "+IntToString(nAmount));
...
//DoDebug("x2_s3_flamingd: nMetaMagic = "+ IntToString(nMetaMagic)+", applying fire damage (1d4 + "+IntToString(nAmount)+") = "+IntToString(nDmg)+" to target "+GetName(oTarget));
