...
int X2_UMD()
{
    ...
    // -------------------------------------------------------------------------
    // Wizards and Sorcerors are supposed to cast scrolls via spellcraft   !! SO ARE CLERICS AND DRUIDS! (change - Lockindal)
    // -------------------------------------------------------------------------
    if (GetLevelByClass(CLASS_TYPE_WIZARD,oCaster) >0 ||GetLevelByClass(CLASS_TYPE_SORCERER,oCaster)  >0 || nBard>0 || GetLevelByClass(CLASS_TYPE_CLERIC, oCaster) > 0 || GetLevelByClass(CLASS_TYPE_DRUID, oCaster) > 0)
    {
        // -------------------------------------------------------------------------
