...
int GetSpontaneousHealBurnableSpell(object oCaster)
{
    ...
    nBurnableSpell = GetBestL1Spell(oCaster, nBurnableSpell);
...
void main()
{
    ...
    {
        FloatingTextStringOnCreature("You have no level 1 spells left to trade for sacred stealth.", oCaster, FALSE);
        return;
    }

    //We got a spell to burn. So we burn it off, then do the spontaneous heal!
    DecrementRemainingSpellUses(oCaster, nBurnableSpell);
    SacredStealthLvl1(oCaster);
