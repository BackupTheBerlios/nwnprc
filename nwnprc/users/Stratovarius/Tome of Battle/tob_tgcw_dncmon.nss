...
void main()
{
        ...
        if(move.bCanManeuver)
        {
                ...
                PerformAttack(oTarget, oInitiator, eNone, 0.0, 0, 0, 0, "Dancing Mongoose Hit", "Dancing Mongoose Miss", OBJECT_INVALID, OBJECT_INVALID, FALSE);
                
                object oOffHand = GetItemInSlot(INVENTORY_SLOT_LEFTHAND, oInitiator);
                
                if(IPGetIsMeleeWeapon(oOffHand))
                {
                        PerformAttack(oTarget, oInitiator, eNone, 0.0, 0, 0, 0, "Dancing Mongoose Hit", "Dancing Mongoose Miss", OBJECT_INVALID, OBJECT_INVALID, TRUE);
                }
        }
}
