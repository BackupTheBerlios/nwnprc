...
void main()
{
    ...
    if(GetIsSkillSuccessful(oTrueSpeaker, SKILL_TRUESPEAK, nDC))
    {
 	...
 	effect eLink = EffectLinkEffects(EffectSkillIncrease(SKILL_OPEN_LOCK, (nClass/3)), oTrueSpeaker), EffectVisualEffect(VFX_DUR_HEARD));
 	eLink = EffectLinkEffects(eLink, EffectSkillIncrease(SKILL_DISABLE_TRAP, (nClass/3)), oTrueSpeaker));
 	eLink = EffectLinkEffects(eLink, EffectSkillIncrease(SKILL_CRAFT_WEAPON, (nClass/3)), oTrueSpeaker));
 	eLink = EffectLinkEffects(eLink, EffectSkillIncrease(SKILL_CRAFT_TRAP, (nClass/3)), oTrueSpeaker));
 	eLink = EffectLinkEffects(eLink, EffectSkillIncrease(SKILL_CRAFT_ARMOR, (nClass/3)), oTrueSpeaker));
