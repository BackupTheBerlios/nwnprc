	...
	if (!nSRCheck)
        {
        	...
        	if (utter.nSpellId == UTTER_ANALYZE_ITEM_HOT /* Example Utterance */)
        	{
        		DoAgitateMetal(oTrueSpeaker, oTarget, VFX_COM_HIT_FIRE, 1, DAMAGE_TYPE_FIRE, nRounds);
        	}
        	// The REVERSE effect of the Utterance goes here
        	else /* Effects of UTTER_ANALYZE_ITEM_CHILL would be here */
        	{
...
void DoAgitateMetal(object oTrueSpeaker, object oTarget, int nVFX, int nBeats, int nDamageType, int nRounds)
{
	...
	// Extended
       	...
       	// Impact VFX 
        utter.eLink2 = EffectLinkEffects(EffectVisualEffect(nVFX), EffectDamage(nDamage, nDamageType));
