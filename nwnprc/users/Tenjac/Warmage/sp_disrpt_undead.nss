...
void main()
{
        ...
        int nType = PRCGetRacialType(oTarget);
                ...
                if(nType == RACIAL_TYPE_UNDEAD)
                {
                        if(!MyPRCResistSpell(oPC, oTarget, nCasterLvl + SPGetPenetr()))
                        {
                                if(nSPell == SPELL_GREATER_DISRUPT_UNDEAD)
                                {
                                        ...
                                        //Apply Damage
                                        SPApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(nDam, DAMAGE_TYPE_POSITIVE));
                                        
                                        //if enough to kill target, bounce
                                        if(nDam >= nHP)
                                        {
                                                location lLoc = GetLocation(oTarget);
                                                int oTarget2 = GetFirstObjectInShape(SHAPE_SPHERE, FeetToMeters(15.0), lLoc, TRUE);
                                ...
                                else SPApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(nDam, DAMAGE_TYPE_POSITIVE));
                        }
                }
        }
