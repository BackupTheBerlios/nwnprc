...
!define APPNAME "PRC Companion"
!define APPNAMEANDVERSION "PRC Companion ${PRCINSTALLVERSION}"
...
InstallDir "$PROGRAMFILES\PRC Companion"
InstallDirRegKey HKLM "Software\${APPNAME}" ""
OutFile "Setup PRC Companion ${PRCVERSION}.exe"
Var NWNVERSION
Var NWNPATH
Var NWNPRCPATH
Var NWNMINVERSION
...
!define MUI_FINISHPAGE_TEXT "The PRC Companion is now installed. "

!define MUI_WELCOMEPAGE_TEXT "The PRC Companion will now be installed into your installation of Neverwinter Nights.\r\n\r\n\r\nNeverwinter Nights version $NWNVERSION is currently installed at $NWNPATH."
...
Section "PRC Pack" Section1
	...
	CreateShortCut "$SMPROGRAMS\PRC Pack\Server Uninstall.lnk" "$NWNPRCPATH\PRCPack\Companion Uninstall.exe"
...
Section -FinishSection
	...
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${APPNAME}" "UninstallString" "$NWNPRCPATH\PRCPack\Companion Uninstall.exe"
	WriteUninstaller "$NWNPRCPATH\PRCPack\Companion Uninstall.exe"
...
Section Uninstall
	...
	; Delete self
	Delete "$NWNPRCPATH\PRCPack\Companion Uninstall.exe"

	; Delete Shortcuts
	Delete "$SMPROGRAMS\PRC Pack\Companion Uninstall.lnk"

	; Clean up PRC Pack
        Delete "$NWNPATH\hak\prcc.hak"
        Delete "$NWNPATH\hak\prcc1.hak"
        Delete "$NWNPATH\hak\prcc2.hak"
        Delete "$NWNPATH\hak\prccc1.hak"
        Delete "$NWNPATH\hak\prccc2.hak"
        Delete "$NWNPATH\hak\prccomp.hak"
        Delete "$NWNPATH\hak\prcc.hif"
        Delete "$NWNPATH\hak\prcc1.hif"
        Delete "$NWNPATH\hak\prcc2.hif"
        Delete "$NWNPATH\hak\prccc1.hif"
        Delete "$NWNPATH\hak\prccc2.hif"
        Delete "$NWNPATH\tlk\prccep.tlk"
